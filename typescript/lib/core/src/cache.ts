...
export class MemoryCache implements ICache {
  ...
  get<T>(key: string): Promise<Result<T, CacheError>> {
    ...
    return Promise.resolve(success(entry.value as T));
  }
  ...
  size(): Promise<Result<number, CacheError>> {
    ...
    return Promise.resolve(success(this.stats.size));
  }
  ...
  keys(pattern?: string): Promise<Result<string[], CacheError>> {
    ...
    if (!pattern) {
      return Promise.resolve(success(allKeys));
    }
    ...
    return Promise.resolve(success(matchedKeys));
  }
  ...
  async getOrSet<T>(
    ...
    if (isFailure(factoryResult)) {
      return factoryResult;
    }

    // Cache the factory result using proper Result<T> pattern
    return match(
      async (value) => {
        const setResult = await this.set(key, value, ttl);
        if (isFailure(setResult)) {
      ...
      (error) => Promise.resolve(failure(error as CacheError)),
      factoryResult,
    );
  }
...
export class RedisCache implements ICache {
  ...
  async get<T>(key: string): Promise<Result<T, CacheError>> {
    ...
    // Handle cache miss and success cases
    return match(
      (value) => {
        if (value === null) {
          // Cache miss case
        ...
        // Cache hit case
        this.stats.hits++;
        this.events.emit("hit", key, value);
        return success(value);
      },
      (error) => failure(error as CacheError),
      result,
    );
  }
...
export const createCache = (config: CacheConfig): Result<ICache, CacheError> => {
  switch (config.backend) {
    ...
    default:
      return failure(
        cacheError(`Unsupported cache backend: ${String(config.backend)}`, {
...
export const cacheAside = async <T>(
  ...
  return fromAsyncTryCatch(
    ...
    (error) =>
      cacheError(`Cache-aside loader failed: ${String(error)}`, {
