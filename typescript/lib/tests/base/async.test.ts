...
describe("Async Transformation Operations", () => {
  describe("flatMapAsync", () => {
    it("preserves failure without calling function", async () => {
      ...
      const fn = vi.fn((x: number) => Promise.resolve(success(x * 2)));
    ...
    it("satisfies functor laws asynchronously", async () => {
      ...
      const identity = (x: number) => Promise.resolve(success(x));
  ...
  describe("mapAsync", () => {
    it("preserves failure without calling function", async () => {
      const error = createError();
      const failureResult = failure(error);
      const fn = vi.fn((_x: number) => Promise.resolve(_x * 2));
  ...
  describe("matchAsync", () => {
    it("calls success handler for success result", async () => {
      ...
      const onSuccess = vi.fn((x: number) => Promise.resolve(`Success: ${x}`));
      const onError = vi.fn((e: QiError) => Promise.resolve(`Error: ${e.message}`));
    ...
    it("handles async handlers with delay", async () => {
      ...
      const onError = (e: QiError) => Promise.resolve(`Error: ${e.message}`);
  ...
  it("flatMapAsync satisfies monad left identity law", async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer(), async (value) => {
        ...
        const fn = (x: number) => Promise.resolve(success(x * 2));
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(fc.integer(), { minLength: 1, maxLength: 5 }), async (values) => {
        ...
        const promises = values.map((value) =>
          delay(Math.random() * 10).then(() => success(value)),
        );
